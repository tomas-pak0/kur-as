...
export async function POST(req: Request) {
  ...
  try {
    if (body.action === "view") {
      const row = await env.DB.prepare("SELECT latitude,longitude,accuracy,updated_at,active FROM shares WHERE viewer_hash=?").bind(secret).first();
    ...
    if (body.action === "update") {
      ...
      const result = await env.DB.prepare("UPDATE shares SET latitude=?,longitude=?,accuracy=?,updated_at=? WHERE editor_hash=? AND active=1")
        .bind(lat, lon, acc, Date.now(), secret).run();
      return result.meta.changes ? reply({ ok: true }) : reply({ error: "Bendrinimas išjungtas" }, 410);
    }
    if (body.action === "stop") {
      const result = await env.DB.prepare("UPDATE shares SET latitude=NULL,longitude=NULL,accuracy=NULL,updated_at=NULL,active=0 WHERE editor_hash=? AND active=1")
        .bind(secret).run();
