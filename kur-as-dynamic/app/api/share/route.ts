...
export async function POST(req:Request) {
  ...
  if(body.action==="create") {
    ...
    try {
      await db().prepare("INSERT INTO shares(id,editor_hash,viewer_hash,created_at,active) VALUES(?,?,?,?,1)")
  ...
  if(typeof secret!=="string" || !tokenPattern.test(secret))return reply({error:"Nuoroda negalioja"},404);
  const secretHash=await hash(secret);
  if(body.action==="view") {
    const row=await db().prepare("SELECT latitude,longitude,accuracy,updated_at,active FROM shares WHERE viewer_hash=?").bind(secretHash).first();
    if(!row || row.active!==1)return reply({error:"Bendrinimas išjungtas"},410);
    return reply({latitude:row.latitude,longitude:row.longitude,accuracy:row.accuracy,updatedAt:row.updated_at});
  }
  if(body.action==="update") {
    ...
    const result=await db().prepare("UPDATE shares SET latitude=?,longitude=?,accuracy=?,updated_at=? WHERE editor_hash=? AND active=1")
      .bind(lat,lon,acc,Date.now(),secretHash).run();
    return result.meta.changes?reply({ok:true}):reply({error:"Bendrinimas išjungtas"},410);
  }
  if(body.action==="stop") {
    const result=await db().prepare("UPDATE shares SET latitude=NULL,longitude=NULL,accuracy=NULL,updated_at=NULL,active=0 WHERE editor_hash=? AND active=1")
      .bind(secretHash).run();
